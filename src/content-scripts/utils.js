...
function getCaretPosition(ctrl) {
  if (isEditable(ctrl)) {
    ...
    return caretPos;
  } else if (ctrl.isContentEditable) {
    // In case it's a div with contenteditable set to true
  ...
  if (isElement(obj) && typeof charCombination === "string") {
    if (isEditable(obj)) {
      ...
      deepestElement.innerText = deepestElement.innerText.substring(
        0,
        deepestElement.innerText.length - 1
      );

      deepestElement.innerText =
        deepestElement.innerText + LETTERS_MAPPING.latin[charCombination];
...
function findDeepest(elem) {
  ...
  if (result.maxDepth > 0)
    console.log(
      result.deepestElem.tagName +
        "." +
        result.maxDepth +
        "." +
        result.deepestElem.innerText
    );
}
